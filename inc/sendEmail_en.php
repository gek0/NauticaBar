...
if($_POST) {
   ...
   $contact_captcha = trim(stripslashes($_POST['contactCaptcha']));
    ...
    {
        $error['captcha'] = "Verification code is incorrect. Refresh the page.";
    }
	...
	if (!preg_match('/^[a-z0-9&\'\.\-_\+]+@[a-z0-9\-]+\.([a-z0-9\-]+\.)*+[a-z]{2}/is', $email)) {
		$error['email'] = "E-mail address is not valid.";
	}
	// Check Message
	if (strlen($contact_message) < 15) {
		$error['message'] = "Message is mandatory, 15 chars is minimum.";
	}
   // Subject
   ...
   if (!$error) {
	  ...
	  if ($mail) { echo "E-mail has been sent successfully."; }
